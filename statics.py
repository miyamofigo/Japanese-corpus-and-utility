...
It is a kind of K-means clustering function.
	...
	clusters = [[random.random() * (ranges[i][1] - ranges[i][0]) + ranges[i][0]
							 for i in range(len(rows[0])) for j in range(k)] for j in range(k)]
	latmatches = None

	for t in range(100):
		print 'Iteration %d' % t 
		bestmatche = [[] for in range(k)]
		...
		for j in range(len(rows)):
			...
			if bestmatches == latmatches:
				break
			latmatches = bestmatches
